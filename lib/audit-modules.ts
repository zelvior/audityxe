...
import { BrokenLinkResult, ImageSampleResult, AdsTxtResult, OgImageResult, ServerHardeningResult, SourceMapExposureResult } from "./network-checks";
...
export function buildAuditModules(ctx: ModuleContext): AuditModule[] {
      ...
      [
        ...
        s.security.corsAllowsAnyOrigin
          ? s.security.corsAllowsCredentialsWithWildcard
            ? fail("CORS configuration", "Access-Control-Allow-Origin: * combined with allow-credentials: true — a serious CORS misconfiguration.")
  ...
  if (!s.security.finalIsHttps) {
    tlsFindings.push(fail("HTTPS availability", "Site is not served over HTTPS — no TLS certificate to inspect."));
  } else if (!tls.fetched) {
    ...
    tlsFindings.push(
      tls.isSelfSigned
        ? fail("Certificate issuer", "Certificate appears self-signed (issuer CN matches subject CN).")
  ...
  if (!sh.checked) {
    ...
    shFindings.push(
      sh.exposedPaths.length > 0
        ? fail("Exposed configuration files", `Publicly accessible: ${sh.exposedPaths.map((p) => p.path).join(", ")}.`)
    ...
    shFindings.push(
      sh.directoryListingDetected
        ? fail("Directory listing", "A directory listing was found exposed on this server.")
  ...
  modules.push(makeModule("dns-security", "DNS Security (CAA & Subdomain Takeover)", "Live DNS checks for certificate-issuance restrictions and dangling CNAME takeover risk.", dnsFindings));
  ...
  const sriFindings: AuditModuleFinding[] = [
    ...
    !smExp.checked
      ...
      : warn("Source map exposure", `${smExp.exposedSourceMaps.length} publicly accessible .js.map file(s) found — can reveal original, unminified source code.`),
  ];
