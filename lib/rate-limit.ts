...
export async function ensureUserDoc(identity: DecodedIdentity, displayName?: string | null): Promise<void> {
  ...
  if (!snap.exists) {
    ...
    // Fire-and-forget: dashboard metrics and admin search must never
    // block or fail a real user's sign-in.
    recordDailyEvent("newUsers").catch(() => {});
    bloomAddTokens([identity.uid, identity.email || "", displayName || ""]).catch(() => {});
  } else {
    // Cheap, non-blocking "last seen" touch so the Active Users metric
    // (users active in the last 30 days) reflects real usage without a
...
export async function checkAndIncrementUsage(uid: string): Promise<UsageResult> {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (used >= limit) {
      incrementCounter("rateLimitHitsAllTime").catch(() => {});
      recordDailyEvent("rateLimitHits").catch(() => {});
      return {
        allowed: false,
        used,
        limit,
