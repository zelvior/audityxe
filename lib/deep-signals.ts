...
export interface VibeCodedSignals {
  ...
  largeRadiusHints: number; // 20px-24px+ rounded corners
...
const ECOMMERCE_PATTERNS: [RegExp, string][] = [
  [/cdn\.shopify\.com|Shopify\.theme/i, "Shopify"],
  [/bigcommerce\.com/i, "BigCommerce"],
  [/magento|Mage\.Cookies/i, "Magento"],
  [/prestashop/i, "PrestaShop"],
...
const CSS_FRAMEWORK_PATTERNS: [RegExp, string][] = [
...
const PAGE_BUILDER_PATTERNS: [RegExp, string][] = [
...
const AB_TESTING_PATTERNS: [RegExp, string][] = [
...
const COOKIE_CONSENT_PATTERNS: [RegExp, string][] = [
...
const HOSTING_HEADER_PATTERNS: [RegExp, string][] = [
  [/^vercel$/i, "Vercel"],
...
export function extractDeepSignals(html: string, serverHeaderValue?: string | null, xRobotsTagValue?: string | null): DeepSignals {
  ...
  let detectedCMSVersion: string | null = null;
  ...
  if (!hostingProvider) {
    if (/\.vercel\.app|_vercel\//i.test(html)) hostingProvider = "Vercel";
    else if (/netlify\.app|__netlify/i.test(html)) hostingProvider = "Netlify";
    else if (/\.pages\.dev\b/i.test(html)) hostingProvider = "Cloudflare Pages";
  }
  ...
  if (detectedCMS) corroboratingSignals++;
  if (generator) corroboratingSignals++;
  if (detectedCMSVersion) corroboratingSignals++;
  if (detectedFrameworks.length && detectedCMS) corroboratingSignals++; // e.g. headless CMS + framework
  const confidence: TechStackSignals["confidence"] =
    corroboratingSignals >= 2 ? "high" : corroboratingSignals === 1 ? "medium" : "low";
  ...
  const structuredData: StructuredDataSignals = {
    ...
    // Speakable is a nested `speakable` property on WebPage/Article, not
    // its own top-level @type, so it isn't caught by the `types` walk
    // above — a raw substring check on the block's own JSON is simpler
    // and correct here, same way a parse-error page can't be walked at
    // all but can still be substring-matched.
  ...
  // "Direct-answer lead": the page's very first h1/h2 immediately
  // followed by a concise (~40–320 char) paragraph, before anything
  // else — the classic definition-first pattern that lets an AI answer
  // engine or featured-snippet extractor lift the opening as a direct
  // answer without having to synthesize one from scattered content.
  ...
  const emojiInHeadingOrButtonCount = headingAndButtonText.reduce(
    (sum, t) => sum + (t.match(emojiRe) || []).length,
    0
  );

  const gradientDeclarationCount = (styleBlocks.match(/linear-gradient|radial-gradient|conic-gradient/gi) || []).length;
  const purpleBlueGradientHints = (
    styleBlocks.match(/linear-gradient\([^)]*(?:purple|violet|indigo|#7[0-9a-f]{5}|#8[0-9a-f]{5}|#9[0-9a-f]{5})[^)]*(?:blue|cyan|#[0-3][0-9a-f]{5}|#4[0-9a-f]{5})[^)]*\)/gi) || []
  ).length;
  ...
  const largeRadiusHints = [...styleBlocks.matchAll(/border-radius\s*:\s*(\d+(?:\.\d+)?)px/gi)].filter(
    (m) => Number(m[1]) >= 20 && Number(m[1]) <= 32
  ).length;
  ...
  const fontFamilies = new Set<string>(
    [...styleBlocks.matchAll(/font-family\s*:\s*([^;]+);/gi)].map((m) =>
      m[1].split(",")[0].replace(/["']/g, "").trim().toLowerCase()
    )
  );
  const distinctFontFamilyCount = fontFamilies.size;
  const interFontOnlyHint = fontFamilies.size === 1 && fontFamilies.has("inter");
  const spaceGroteskInstrumentSerifPairHint =
    fontFamilies.has("space grotesk") && fontFamilies.has("instrument serif");

  const emDashCount = (html.match(/—/g) || []).length;
  const loremIpsumHint = /lorem ipsum/i.test(html);

  const vibeCoded: VibeCodedSignals = {
  ...
  const collectOrigins = (re: RegExp): string[] => {
  ...
  const fontOrigins = Array.from(
    new Set([...rawStyleOrigins.filter((o) => FONT_HOSTS.test(o)), ...preconnectOrigins.filter((o) => FONT_HOSTS.test(o))])
  );
  ...
  const connectOrigins = rawScriptOrigins.filter((o) =>
    /google-analytics\.com|analytics\.google\.com|googletagmanager\.com|segment\.(io|com)|sentry\.io|mixpanel\.com|amplitude\.com/i.test(o)
  );

  const cspOrigins: CspOriginSignals = {
