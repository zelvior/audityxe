 ...
 */
export async function saveLastAuditScore(rawHost: string, overall: number): Promise<void> {
  // `rawHost` may be a bare hostname or a full URL (finalUrl) depending on
  // the caller — normalize identically to how /api/badge/[domain] derives
  // its lookup key, or a re-audit silently writes under a different doc
  // (protocol/www/trailing-slash differences) and the badge never updates.
