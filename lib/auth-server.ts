...
export async function requireAuth(req: NextRequest, options: RequireAuthOptions = {}): Promise<DecodedIdentity> {
  ...
  // Enforced on every authenticated request, not just the audit route —
  // a banned/suspended account gets 403'd everywhere the moment the
  ...
  if (moderation.status === "suspended") {
    const untilText = moderation.until ? new Date(moderation.until).toLocaleDateString() : "further notice";
    throw new AuthError(
      moderation.reason
        ? `Your account is suspended until ${untilText}: ${moderation.reason}`
        : `Your account is suspended until ${untilText}.`,
      403,
      "ACCOUNT_SUSPENDED"
    );
  }
