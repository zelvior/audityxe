...
import { checkBrokenLinks, checkImageSample, checkAdsTxt, checkOgImage, checkServerHardening, checkSourceMapExposure, checkSecurityTxt, checkFaviconManifest, checkAssetWeights } from "./network-checks";
...
import { checkEmailAuthDns } from "./dns-email-auth";
import { checkDnsSecurity } from "./dns-security";
import { classifySiteContext } from "./site-context";
import { checkLegalPages } from "./legal-pages";
...
export interface SecuritySignals {
  ...
  referrerPolicyIsWeak: boolean; // e.g. "unsafe-url"
...
function extractSignals(html: string, finalUrl: string): Signals {
  ...
  const cleanHeadingText = (raw: string) => raw.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().toLowerCase();
  const dupeTexts = (matches: RegExpMatchArray[]) => {
  ...
  const iframeMissingDimensions = iframeTags.filter(
  ...
  const syllableCount = words.reduce((sum: number, w: string) => sum + countSyllables(w), 0);
  ...
  return {
    ...
    security: {
      ...
      formsWithInsecureAction: finalUrl.startsWith("https://")
        ? [...html.matchAll(/<form\b[^>]*action\s*=\s*["'](http:\/\/[^"']+)["']/gi)].filter(
            (m) => !/^http:\/\/(localhost|127\.0\.0\.1)/i.test(m[1])
          ).length
        : 0,
...
function stackAwareHeaderFix(
  s: Signals,
  deep: import("./deep-signals").DeepSignals
): { fix: string; snippet: string } {
...
function buildIntelligentCsp(deep: import("./deep-signals").DeepSignals): { fix: string; snippet: string } {
  const c = deep.cspOrigins;
  const directive = (name: string, origins: string[]) =>
    origins.length ? `${name} 'self' ${origins.join(" ")};` : `${name} 'self';`;
...
{ "xPost": "...", "linkedinPost": "..." }`;
  ...
  const result = await generateJsonForTask<PromoCopy>("PROMO", prompt, {
    temperature: 0.9,
...
Return ONLY valid JSON (no markdown fences) matching this exact shape:
{
  ...
  const result = await generateJsonForTask<BannerDesign>("BANNER", prompt, {
    temperature: 0.75,
...
async function runAuditInner(
  rawUrl: string,
  competitorRawUrl: string | undefined,
  options: AuditOptions
): Promise<AuditResult> {
  ...
  const siteContext = classifySiteContext(primary.html, primary.signals, deepSignals);
  ...
  const [verdictCopy, promoCopy, bannerDesign, brokenLinks, imageSample, adsTxt, ogImage, pageSpeed, emailAuth, serverHardening, dnsSecurity, sourceMapExposure, securityTxt, faviconManifest, assetWeights, legalPages, siteCrawl] = await Promise.all([
