...
async function runAuditInner(
  ...
  const [verdictCopy, promoCopy, bannerDesign, brokenLinks, imageSample, adsTxt, ogImage, pageSpeed, emailAuth, serverHardening, dnsSecurity, sourceMapExposure, securityTxt, faviconManifest, assetWeights] = await Promise.all([
    ...
    includePageSpeed ? fetchPageSpeedInsights(primary.finalUrl) : Promise.resolve(EMPTY_PAGESPEED_SUMMARY),
