...
export async function checkCookieFlags(origin: string): Promise<CookieAuditResult> {
  const result: CookieAuditResult = { checked: false, cookieCount: 0, missingSecure: [], missingHttpOnly: [], missingSameSite: [] };
  try {
    await assertSafeUrl(origin);
  } catch {
    return result;
  ...
  try {
    ...
    const setCookieHeaders = raw.length ? raw : (res.headers.get("set-cookie") ? [res.headers.get("set-cookie") as string] : []);
    ...
    for (const line of setCookieHeaders) {
      ...
      if (!/;\s*secure/i.test(line)) missingSecure.push(name);
      if (!/;\s*httponly/i.test(line)) missingHttpOnly.push(name);
      if (!/;\s*samesite/i.test(line)) missingSameSite.push(name);
    }
...
export async function checkRedirectChain(rawUrl: string): Promise<RedirectChainResult> {
  const result: RedirectChainResult = { checked: false, hopCount: 0, chain: [], isLoop: false, excessiveHops: false, httpsUpgradeMissing: false };
  const chain: string[] = [];
