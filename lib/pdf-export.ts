...
function lastAutoTableYOr(doc: jsPDF, fallback: number): number {
...
export function generateAuditPdf(result: AuditResult) {
  ...
  result.modules.forEach((m: AuditModule) => {
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        3: { cellWidth: 65 },
        4: { cellWidth: "auto", textColor: [107, 107, 120] },
      },
      didParseCell: (data) => {
  ...
  if (result.fixes.length) {
    ...
    result.fixes.forEach((fix) => {
      const cursorY = lastAutoTableYOr(doc, y);
      const startY = cursorY > 250 ? 20 : cursorY + 8;
      if (cursorY > 250) doc.addPage();
      autoTable(doc, {
        startY,
        margin: { left: MARGIN, right: MARGIN },
        theme: "plain",
