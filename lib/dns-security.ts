...
export async function checkDnsSecurity(hostname: string, timeoutMs = 6000): Promise<DnsSecurityInfo> {
  ...
  try {
    let hasCaaRecords = false;
    let caaIssuers: string[] = [];
    try {
      const caaRecords = await withTimeout(dns.resolveCaa(hostname));
      hasCaaRecords = caaRecords.length > 0;
      caaIssuers = caaRecords.map((r: { issue?: string; issuewild?: string }) => r.issue || r.issuewild || "").filter(Boolean);
    } catch {
      /* no CAA records — leave defaults */
    }

    let cnameTarget: string | null = null;
    try {
      const cnames = await withTimeout(dns.resolveCname(hostname));
      cnameTarget = cnames[0] || null;
    } catch {
      /* no CNAME — apex/A-record domain, nothing to check */
    }
    ...
    const dnssecEnabled = await checkDnssec(hostname, timeoutMs).catch(() => false);
