...
function warn(label, detail, evidence, severity = "medium", confidence) {
    return { label, status: "warn", detail, evidence, severity, confidence };
}
function fail(label, detail, evidence, severity = "high", confidence) {
...
function buildLighthouseModule(pageSpeed) {
    ...
    if (!pageSpeed.fetched) {
        return makeModule("lighthouse", "Lighthouse Audit", "Real browser-rendered performance, accessibility, best-practices, and SEO scores from a live Google PageSpeed Insights run.", [unknown("PageSpeed Insights run failed", pageSpeed.errorMessage || "Unknown error — the request did not return usable data.")]);
    }
    const findings = [];
    const category = (label, value) => {
        ...
        if (value >= 90)
            findings.push(pass(`${label} score`, `${value}/100 — Lighthouse's \"good\" band (90+).`, `Lighthouse ${label} category: ${value}/100`));
        else if (value >= 50)
    ...
    const vital = (label, value, unit, good, poor) => {
        if (value == null) {
        ...
        if (value <= good)
            findings.push(pass(label, `${formatted} — within Google's \"good\" threshold (≤ ${good}${unit}).`, `Lab ${label}: ${formatted}`));
        else if (value <= poor)
            findings.push(warn(label, `${formatted} — needs improvement (\"good\" is ≤ ${good}${unit}, \"poor\" is > ${poor}${unit}).`, `Lab ${label}: ${formatted}`, "medium"));
        else
            findings.push(fail(label, `${formatted} — in Google's \"poor\" band (> ${poor}${unit}).`, `Lab ${label}: ${formatted}`, "high"));
    };
    ...
    if (pageSpeed.fieldData?.available) {
        ...
        if (fd.overallCategory === "FAST")
            findings.push(pass(label, "Actual Chrome users report a FAST experience over the last 28 days — the higher-confidence real-world signal.", `CrUX overall category: FAST`));
        ...
        findings.push(unknown("Real-world Core Web Vitals", "No CrUX field data available for this origin — not enough recorded Chrome traffic for Google to report on. The lab metrics above are the best available signal."));
    }
...
function buildAuditModules(ctx) {
    ...
    const modules = [];
    /* 1. SEO ────────────────────────────────────────────────────────── */
    modules.push(makeModule("seo", "SEO", "Crawlability, indexability, and on-page search signals.", [
        s.title
    ...
    /* 2. Performance ────────────────────────────────────────────────── */
    modules.push(makeModule("performance", "Performance", "Real measured response time, page weight, and render-blocking resources.", [
        ...
        (() => {
            ...
            if (!aw.checked || total === 0)
                return pass("Asset payload distribution", "No external script/stylesheet/image assets to weigh.");
    ...
    /* 3. Security headers ───────────────────────────────────────────── */
    modules.push(makeModule("security-headers", "Security Headers", "Real HTTP response headers that protect visitors and the site itself.", [
        ...
        s.security.cookieCount === 0
            ...
            : s.security.cookiesMissingSecure === 0 && s.security.cookiesMissingHttpOnly === 0 && s.security.cookiesMissingSameSite === 0
                ...
                : warn("Cookie security", `${s.security.cookieCount} cookie(s) set — ${s.security.cookiesMissingSecure} missing Secure, ${s.security.cookiesMissingHttpOnly} missing HttpOnly, ${s.security.cookiesMissingSameSite} missing SameSite.`),
        s.security.cookieCount === 0
            ...
            : (() => {
                ...
                if (s.security.cookiesTrackingSuspectedCount > 0)
                    parts.push(`${s.security.cookiesTrackingSuspectedCount} likely tracking`);
    ...
    if (!s.security.finalIsHttps) {
        ...
        tlsFindings.push(tls.isExpired
        ...
        tlsFindings.push(tls.isSelfSigned
        ...
        tlsFindings.push(tls.hostnameMatches
        ...
        tlsFindings.push(tls.isWeakProtocol
    ...
    if (!ea.fetched) {
        ...
        if (ea.hasSpf) {
            emailFindings.push(ea.spfExceedsLookupLimit
                ? fail("SPF lookup limit", `~${ea.spfLookupCount} DNS-lookup mechanisms found in the SPF record, over RFC 7208's hard limit of 10 \u2014 receiving mail servers will return a permerror and may treat SPF as unset entirely.`, undefined, "high")
                : pass("SPF lookup limit", `~${ea.spfLookupCount} of the 10 allowed DNS-lookup mechanisms used.`));
        }
        emailFindings.push(ea.hasDmarc
        ...
        emailFindings.push(ea.hasDkimOnAnyCommonSelector
    ...
    if (!sh.checked) {
        ...
        shFindings.push(sh.exposesDangerousMethods
        ...
        shFindings.push(sh.exposedPaths.length > 0
        ...
        shFindings.push(sh.directoryListingDetected
    ...
    if (!dnsSec.fetched) {
        ...
        dnsFindings.push(dnsSec.hasCaaRecords
        ...
        dnsFindings.push(dnsSec.dnssecEnabled
        ...
        if (dnsSec.cnameTarget) {
            dnsFindings.push(dnsSec.possibleDanglingCname
                ? fail("Dangling CNAME / subdomain takeover", `CNAME points to ${dnsSec.cnameTarget} (${dnsSec.cnamePointsToKnownService}), which does not resolve — this pattern is consistent with a claimable, unclaimed third-party resource.`, undefined, "critical")
        ...
        dnsFindings.push(dnsSec.nsRecords.length > 0
        ...
        dnsFindings.push(dnsSec.hasSoaRecord
        ...
        dnsFindings.push(dnsSec.mxRecords.length > 0
    ...
    const sriFindings = [
        ...
        d.linkSafety.blankTargetLinkCount === 0
            ...
            : d.linkSafety.blankTargetMissingNoopener === 0
                ...
                : warn("target=\"_blank\" safety", `${d.linkSafety.blankTargetMissingNoopener} of ${d.linkSafety.blankTargetLinkCount} target="_blank" link(s) are missing rel="noopener" — the opened page gets a live window.opener reference back (a tabnabbing risk).`),
    ];
    ...
    const rankLabel = { necessary: "Necessary", recommended: "Recommended", skippable: "Skippable" };
    ...
    const rankConfidence = site.confidence === "high" ? "high" : site.confidence === "medium" ? "medium" : "low";
    const legalFindings = legal.pages.map((p) => {
    ...
    const legalSummary = `Detected as: ${site.label} (${site.confidence} confidence)${site.reasons.length ? ` (based on ${site.reasons.join("; ")})` : ""}. Each legal/trust page below is ranked necessary, recommended, or skippable specifically for this site type, not a one-size-fits-all checklist. On low-confidence classifications, treat "necessary" ranks here as a starting point rather than a hard verdict.`;
    ...
    /* 4. Accessibility ──────────────────────────────────────────────── */
    modules.push(makeModule("accessibility", "Accessibility", "Real markup checks for keyboard, screen-reader, and form usability.", [
        ...
        d.accessibility.genericLinkTextCount === 0
            ...
            : warn("Descriptive link text", `${d.accessibility.genericLinkTextCount} link(s) use non-descriptive text like "click here" or "read more" — screen-reader users navigating a page's link list out of context get no clue where these go, and it's a missed SEO signal too.`, undefined, "low"),
        d.accessibility.hasSkipLink
            ...
            : warn("Skip-to-content link", "No skip-to-content link found — keyboard users have to tab through the full navigation on every single page before reaching the main content.", undefined, "low"),
        s.imgTotal === 0
        ...
        d.accessibility.suppressesFocusOutlineWithoutReplacement
            ? fail("Keyboard focus visibility", "CSS sets outline: none/0 with no visible :focus or :focus-visible replacement (box-shadow, border, or outline rule) found — keyboard users can't see which element is focused.", undefined, "high")
            : pass("Keyboard focus visibility", "No focus-outline suppression without a visible replacement was found."),
    ]));
    /* 5. Mobile responsiveness ──────────────────────────────────────── */
    modules.push(makeModule("mobile", "Mobile Responsiveness", "Viewport configuration, touch icons, and responsive CSS signals.", [
        ...
        d.mobile.mediaQueryCount > 0
            ...
            : d.mobile.responsiveClassHintCount > 0
                ? pass("Responsive CSS", `No inline @media rules, but ${d.mobile.responsiveClassHintCount} responsive utility-class usage(s) found (e.g. Tailwind-style sm:/md:/lg: breakpoints) — real evidence of a responsive framework, even though its rules live in an external stylesheet we don't fetch.`)
    ...
    /* 6. UX / UI ─────────────────────────────────────────────────────── */
    modules.push(makeModule("ux-ui", "UX / UI", "Visual hierarchy and conversion-path signals.", [
        s.hasAboveFoldCta
    ...
    vibeFindings.push(vc.purpleBlueGradientHints > 0
    ...
    vibeFindings.push(vc.gradientTextHints > 0
    ...
    vibeFindings.push(vc.glassmorphismHints > 0
    ...
    vibeFindings.push(vc.emojiInHeadingOrButtonCount > 0
    ...
    vibeFindings.push(vc.pillBadgeAboveHeadingHints > 0
    ...
    vibeFindings.push(vc.genericIconRowHints > 0
    ...
    vibeFindings.push(vc.scrollFadeInAnimationHints > 0
    ...
    vibeFindings.push(vc.cursorGlowOrParticleHints > 0
    ...
    vibeFindings.push(vc.hoverOpacityFadeHints > 0
    ...
    vibeFindings.push(vc.grainyTextureHints > 0
    ...
    vibeFindings.push(vc.largeRadiusHints > 0
    ...
    vibeFindings.push(vc.interFontOnlyHint
    ...
    vibeFindings.push(vc.spaceGroteskInstrumentSerifPairHint
    ...
    vibeFindings.push(vc.emDashCount >= 3
    ...
    vibeFindings.push(vc.buzzwordsFound.length > 0
    ...
    const vibeSummary = vibeSignalCount === 0
    ...
    if (d.techStack.detectedCMS) {
        ...
        stackFindings.push(pass("CMS / platform", `No traditional CMS — custom-built with ${d.techStack.detectedFrameworks.join(", ")}, which is a legitimate and common approach.`));
    else
        stackFindings.push(warn("CMS / platform", "No known CMS or JS framework signature detected — stack couldn't be identified.", undefined, "low"));
    if (d.techStack.detectedFrameworks.length > 0)
        ...
        stackFindings.push(warn("Frontend framework", "No known JS framework signature detected.", undefined, "low"));
    ...
    stackFindings.push(d.techStack.jqueryDetected
    ...
    stackFindings.push(d.techStack.detectedAnalytics.length > 0
    ...
    if (d.techStack.detectedCookieConsentTools.length > 0)
        ...
        stackFindings.push(warn("Cookie consent tooling", "Analytics/tag-manager scripts are present but no cookie-consent tool was detected \u2014 worth confirming compliance is handled another way.", undefined, "medium"));
    stackFindings.push(d.techStack.detectedCDNs.length > 0
    ...
    if (d.techStack.hostingProvider)
        stackFindings.push(pass("Hosting provider", `Detected: ${d.techStack.hostingProvider}.`));
    ...
    /* 8. HTML structure ─────────────────────────────────────────────── */
    modules.push(makeModule("html-structure", "HTML Structure", "Document validity and semantic markup quality.", [
        ...
        d.htmlStructure.hasHeaderTag && d.htmlStructure.hasMainTag && d.htmlStructure.hasFooterTag && d.htmlStructure.hasNavTag
            ...
            : warn("Landmark structure", `Missing: ${[
        ...
        d.htmlStructure.commentCount <= 40
            ...
            : warn("HTML comment volume", `${d.htmlStructure.commentCount} HTML comments found — often leftover CMS/debug cruft that adds page weight with no benefit to visitors.`, undefined, "low"),
    ]));
    /* 9. Meta tags ───────────────────────────────────────────────────── */
    modules.push(makeModule("meta-tags", "Meta Tags", "Full audit of head-section metadata.", [
    ...
    modules.push(makeModule("sitemap-robots", "Sitemap & Robots.txt", "Live crawl-control files fetched directly from the site.", [
        s.robotsTxt.fetched
            ? s.robotsTxt.exists
                ? pass("robots.txt exists", `${s.robotsTxt.ruleCount} allow/disallow rule(s).`, `Sent a live GET request to ${s.robotsTxt.checkedUrl} \u2014 received HTTP ${s.robotsTxt.httpStatus}.`)
                : fail("robots.txt exists", "Not found at the domain root.", `Sent a live GET request to ${s.robotsTxt.checkedUrl} \u2014 received HTTP ${s.robotsTxt.httpStatus ?? "no response"}.`)
        ...
        s.sitemap.fetched
            ? s.sitemap.exists
                ? pass("sitemap.xml exists", `Valid XML, ${s.sitemap.urlCount} entr${s.sitemap.urlCount === 1 ? "y" : "ies"}.`, `Sent a live GET request to ${s.sitemap.checkedUrl} \u2014 received HTTP ${s.sitemap.httpStatus}, parsed as valid sitemap XML.`)
                : fail("sitemap.xml exists", "Not found or not valid XML.", `Sent a live GET request to ${s.sitemap.checkedUrl} \u2014 received HTTP ${s.sitemap.httpStatus ?? "no response"}${s.sitemap.httpStatus && s.sitemap.httpStatus < 300 ? ", but the body did not parse as valid sitemap XML" : ""}.`)
    ...
    modules.push(makeModule("structured-data", "Structured Data", "JSON-LD schema.org markup, parsed and validated.", [
        ...
        missingFieldEntries.length === 0
            ...
            : fail("Required fields", missingFieldEntries.map(([type, fields]) => `${type} missing ${fields.join(", ")}`).join("; ")),
    ...
    /* 12. Broken links ──────────────────────────────────────────────── */
    modules.push(makeModule("broken-links", "Broken Links", `Live-checked ${brokenLinks.checked} link(s) found on the page via real HTTP requests.`, [
        ...
        ...(() => {
            ...
            if (unambiguous.length === 0) {
                ...
                findings.push(fail("Broken (404/410/5xx)", unambiguous.map((b) => `${b.url} -> ${b.status}`).join("; ")));
            }
            if (ambiguous.length > 0) {
                findings.push(warn("Blocked automated checks", `${ambiguous.map((b) => `${b.url} -> ${b.status}`).join("; ")} (may be intentionally blocking bots rather than genuinely broken — verify manually).`));
            }
            return findings;
        })(),
        brokenLinks.broken.filter((b) => b.status === 0).length === 0
            ? pass("Unreachable links", "None in the sample.")
            : warn("Unreachable links", `${brokenLinks.broken.filter((b) => b.status === 0).length} link(s) timed out or failed to connect (may be firewalled against bots rather than truly broken).`),
    ]));
    /* 13. Image optimization ────────────────────────────────────────── */
    modules.push(makeModule("images", "Image Optimization", "Format, sizing, and lazy-loading, including a live weight sample.", [
        d.images.total === 0
        ...
        imageSample.oversized.length === 0
            ...
            : fail("Live weight sample", imageSample.oversized.map((o) => `${o.url} — ${o.sizeKb}KB`).join("; ")),
    ]));
    /* 14. Third-party scripts ───────────────────────────────────────── */
    modules.push(makeModule("third-party-scripts", "Third-Party Scripts", `${d.thirdPartyScripts.totalExternalScripts} external script domain(s) detected and categorized.`, [
    ...
    /* 15. Social metadata ───────────────────────────────────────────── */
    modules.push(makeModule("social-metadata", "Social Metadata", "Open Graph and Twitter Card tags, with a live check that the preview image actually loads.", [
        d.socialMeta.ogType && d.socialMeta.ogUrl && s.hasOgTitle && s.hasOgDescription
            ...
            : warn("Open Graph completeness", `Missing: ${[
        ...
        !d.socialMeta.ogImageUrl
            ...
            : !ogImage.checked
                ...
                : ogImage.exists && ogImage.isImage
                    ...
                    : warn("og:image", `Set to "${d.socialMeta.ogImageUrl}" but a live check couldn't confirm it loads as an image — this can be a genuine broken URL, or a CDN blocking automated requests. Verify manually if unsure.`),
    ]));
    ...
    modules.push(makeModule("monetization", "Basic Monetization Setup", "Ad networks, affiliate links, payment processors, and a live ads.txt check.", [
        adsTxt.fetched
            ? adsTxt.exists
    ...
    if (!crawl.crawled) {
        modules.push(makeModule("site-structure", "Site Structure (Multi-Page Crawl)", "Bounded same-origin crawl beyond the homepage for internal linking and orphan-page detection.", [
        ...
        const crawlFindings = [
            pass("Pages sampled", `Crawled ${crawl.pagesCrawled} page(s) same-origin from the homepage${crawl.uncrawledInternalLinkCount > 0 ? ` (${crawl.uncrawledInternalLinkCount} more internal link(s) found but not crawled — sample capped for speed).` : "."}`, crawl.pages.map((p) => p.url).join(", "), "high"),
            crawl.brokenInternalLinks.length === 0
                ...
                : fail("Internal links (extended)", `${crawl.brokenInternalLinks.length} broken internal link(s) found beyond the homepage, e.g. ${crawl.brokenInternalLinks[0].from} → ${crawl.brokenInternalLinks[0].to} (HTTP ${crawl.brokenInternalLinks[0].status}).`, undefined, "high"),
            orphans.length === 0
                ...
                : warn("Orphan pages", `${orphans.length} crawled page(s) have no incoming internal links from other crawled pages, e.g. ${orphans[0].url} — likely undiscoverable by users or crawlers without a direct link/sitemap entry.`, undefined, "medium", "medium"),
            thinPages.length === 0
                ? pass("Thin content (extended)", "No crawled page beyond the homepage has suspiciously little visible text.")
                : warn("Thin content (extended)", `${thinPages.length} crawled page(s) have under 80 words of visible text, e.g. ${thinPages[0].url} (${thinPages[0].wordCount} words) — could be genuinely thin, or could be a JS-rendered page this static crawl can't fully see (see below).`, undefined, "low", "medium"),
            jsRenderedPages.length === 0
                ...
                : warn("JS-rendered content risk", `${jsRenderedPages.length} crawled page(s) show signals consistent with content that only renders after client-side JavaScript runs (${jsRenderedPages[0].jsRenderReasons[0]}) — this audit fetches static HTML only, so word count, SEO text, and content-quality checks on those pages may under-report what a real visitor/crawler with JS enabled would actually see.`, undefined, "low", "low"),
        ];
        modules.push(makeModule("site-structure", "Site Structure (Multi-Page Crawl)", `Bounded same-origin crawl of ${crawl.pagesCrawled} page(s) beyond the homepage for internal linking, orphan-page detection, and a JS-rendering caveat where relevant.`, crawlFindings));
    }
    ...
    if (!cookieFlags.checked) {
        ...
        cookieFindings.push(cookieFlags.missingSecure.length === 0
            ? pass("Secure flag", `All ${cookieFlags.cookieCount} cookie(s) set the Secure flag.`)
            : fail("Secure flag", `${cookieFlags.missingSecure.length} cookie(s) missing Secure: ${cookieFlags.missingSecure.join(", ")}.`, undefined, "high"), cookieFlags.missingHttpOnly.length === 0
            ? pass("HttpOnly flag", "All cookies set the HttpOnly flag.")
            : warn("HttpOnly flag", `${cookieFlags.missingHttpOnly.length} cookie(s) missing HttpOnly: ${cookieFlags.missingHttpOnly.join(", ")}.`, undefined, "medium"), cookieFlags.missingSameSite.length === 0
    ...
    if (!redirectChain.checked) {
        ...
        redirectFindings.push(redirectChain.hopCount === 0
            ...
            : redirectChain.excessiveHops
                ...
                : pass("Redirect hops", `${redirectChain.hopCount} redirect hop(s), within a reasonable range.`, redirectChain.chain.join(" → ")), redirectChain.httpsUpgradeMissing
     ...
     * headings, a direct-answer opening paragraph). ─────────────────── */
    {
        ...
        if (!rb.fetched) {
            ...
            geoFindings.push(warn("AI crawler access", `robots.txt explicitly blocks ${rb.aiBotsBlocked.length} named AI crawler${rb.aiBotsBlocked.length > 1 ? "s" : ""} — content won't be readable for AI-search citations from these.`, rb.aiBotsBlocked.join(", "), "medium"));
        }
        ...
        if (!llms.fetched) {
            ...
            geoFindings.push(warn("llms.txt", "Not found. This is an emerging (not yet universal) convention some AI answer engines use to understand a site — a root-level, plain-markdown summary, the way robots.txt/sitemap.xml serve traditional crawlers.", llms.checkedUrl, "low"));
        }
        const ls = d.linkSafety;
        if (ls.xRobotsTagBlocksIndexing) {
            geoFindings.push(warn("X-Robots-Tag header", "The X-Robots-Tag response header blocks indexing (noindex) — this blocks every crawler, AI included, even if robots.txt and the HTML meta tag look fine.", ls.xRobotsTagValue || undefined, "high"));
        }
        else {
            geoFindings.push(pass("X-Robots-Tag header", "No header-level noindex directive found."));
        }
        if (ls.metaRobotsVsHeaderConflict) {
            geoFindings.push(warn("Meta robots vs. header conflict", "The HTML <meta name=\"robots\"> tag and the X-Robots-Tag HTTP header disagree on whether this page should be indexed — worth resolving so the intent is unambiguous to every crawler.", undefined, "medium"));
        }
