...
exports.EMPTY_PAGESPEED_SUMMARY = void 0;
exports.fetchPageSpeedInsights = fetchPageSpeedInsights;
/**
...
exports.EMPTY_PAGESPEED_SUMMARY = {
    fetched: false,
    attempted: false,
    errorMessage: null,
...
function extractFieldData(data) {
    ...
    const cat = src.overall_category || null;
    return {
        available: true,
...
function sanitizePsiErrorMessage(raw) {
    return raw
        .replace(/https?:\/\/\S+/g, "")
        .replace(/\(\s*\)/g, "")
        .replace(/\s{2,}/g, " ")
...
function extractBestScreenshot(audits) {
    ...
    const fullPage = audits["full-page-screenshot"]
        ?.details?.screenshot?.data;
    if (typeof fullPage === "string" && fullPage.startsWith("data:image"))
        return fullPage;
    const final = audits["final-screenshot"]?.details?.data;
    return typeof final === "string" && final.startsWith("data:image") ? final : null;
}
async function fetchPageSpeedInsightsPrimary(targetUrl, byokApiKey) {
    const empty = { ...exports.EMPTY_PAGESPEED_SUMMARY, attempted: true };
    ...
    try {
        ...
        if (!res.ok) {
            ...
            try {
                ...
                if (typeof msg === "string" && msg.trim())
                    reason = sanitizePsiErrorMessage(msg.trim());
            }
            catch {
            ...
            if (res.status === 400 && apiKey)
                reason = `Invalid PageSpeed Insights API key, or a malformed request. (${reason})`;
            if (res.status === 403) {
                reason =
            ...
            if (res.status === 429)
                reason = `PageSpeed Insights quota exceeded for ${usingByok ? "your API key" : "the site's shared free tier"}. (${reason})`;
        ...
        if (!lh) {
            ...
            return {
                ...empty,
                errorMessage: typeof psiError === "string" && psiError.trim()
        ...
        const pct = (cat) => typeof cat?.score === "number" ? Math.round(cat.score * 100) : null;
        const metricMs = (id) => typeof audits[id]?.numericValue === "number" ? Math.round(audits[id].numericValue) : null;
 ...
 */
async function fetchDesktopScreenshot(targetUrl, apiKey) {
    const params = new URLSearchParams();
    params.set("url", targetUrl);
 ...
 */
async function fetchPageSpeedInsights(targetUrl, byokApiKey) {
    const apiKey = byokApiKey || process.env.PAGESPEED_API_KEY;
