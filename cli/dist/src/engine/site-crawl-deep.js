...
exports.crawlSiteDeep = crawlSiteDeep;
 ...
 */
async function crawlSiteDeep(homepageHtml, homepageUrl) {
    const empty = (error) => ({
        crawled: false,
        error,
        pagesCrawled: 0,
    ...
    const queued = new Set([homepageUrl]);
    ...
    while (queue.length > 0 && visited.size < DEEP_MAX_PAGES) {
        ...
        const fetchedBatch = await runWithConcurrency(batch, DEEP_MAX_CONCURRENT_FETCHES, async ({ url, depth }) => {
            ...
            const hrefs = $("a[href]")
                ...
                .get();
            const links = Array.from(new Set(hrefs.map((h) => resolveInternal(h, url, origin)).filter((u) => !!u))).slice(0, DEEP_MAX_LINKS_SAMPLED_PER_PAGE);
        ...
        for (const { url, depth, page, links } of fetchedBatch) {
            ...
            if (depth < DEEP_MAX_DEPTH) {
                for (const link of links) {
                    ...
                    if (visited.size + queue.length >= DEEP_MAX_PAGES)
                        continue;
                    queued.add(link);
