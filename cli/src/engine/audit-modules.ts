   ...
   * headings, a direct-answer opening paragraph). ─────────────────── */
  {
    ...
    geoFindings.push(
      sd.hasFaqPage
        ...
        : warn(
            "FAQPage schema",
            "No FAQPage structured data found. If this page already has an FAQ-style section, marking it up with FAQPage schema is one of the highest-leverage single changes for AI-answer citability.",
            undefined,
            "low"
          )
    );
    ...
    geoFindings.push(
      ar.questionHeadingCount > 0
        ? pass(
            "Question-phrased headings",
            `${ar.questionHeadingCount} heading${ar.questionHeadingCount > 1 ? "s" : ""} phrased as a direct question (e.g. "${ar.questionHeadingSamples[0]}") — exactly the shape AI Overviews and answer engines pull from.`,
            ar.questionHeadingSamples.join(" · ") || undefined
          )
        : warn(
            "Question-phrased headings",
            "No headings are phrased as a direct question. Rephrasing a section heading as the question it answers (\"How does X work?\" rather than \"How It Works\") makes it far more likely to be lifted verbatim as an AI-generated answer.",
            undefined,
            "low"
          )
    );
    geoFindings.push(
      ar.hasDirectAnswerLead
        ...
        : warn(
            ...
            "The first heading isn't followed by a short, self-contained paragraph. Leading with a 1–2 sentence direct answer right after the main heading (before diving into detail) is the single most citable structure for AI answer engines.",
            undefined,
            "low"
          )
    );
    ...
    modules.push(
      makeModule(
        ...
        "Two distinct questions: can AI answer engines like ChatGPT, Claude, and Perplexity crawl this site at all (GEO), and is its content actually shaped so they can lift a direct, citable answer from it (AEO)?",
        geoFindings
      )
    );
  }
