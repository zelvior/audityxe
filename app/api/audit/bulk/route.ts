...
export async function POST(req: NextRequest) {
  ...
  const results = await runWithConcurrency<string, BulkResultItem>(urls, CONCURRENCY, async (url) => {
    try {
      // Bulk results only surface url/overall/categories (see BulkResultItem
      // below) — promo copy and PageSpeed aren't part of the bulk output,
      // so skip generating them entirely rather than burning 20x BYOK calls
      // for data nobody sees.
      const result = await runAudit(url, undefined, { includePromo: false, includePageSpeed: false });
