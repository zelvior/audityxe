...
import { updateDisplayNameOnRecord, saveByokKey, removeByokKey, getByokInfo } from "@/lib/user-settings";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [byok, plan] = await Promise.all([getByokInfo(identity.uid), getUserPlan(identity.uid)]);
    return NextResponse.json({ byok, plan });
  } catch (err) {
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    let body: { displayName?: string; aiApiKey?: string | null; aiBaseUrl?: string; aiModel?: string };
    try {
      body = await req.json();
    } catch {
    ...
    const byok = await getByokInfo(identity.uid);
    return NextResponse.json({ ok: true, byok });
  } catch (err) {
    if (err instanceof AuthError) {
      return NextResponse.json({ error: err.message }, { status: err.status });
    }
