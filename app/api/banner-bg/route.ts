...
const BANNER_IMAGE_DAILY_LIMIT = 10;
 ...
 *
 * Gated to the Pro plan with its own atomic daily quota, separate from
 * the audit quota — this endpoint has a real compute/bandwidth cost per
 * call, so it's metered like any other paid feature.
 */
export async function GET(req: NextRequest) {
  ...
  if (plan !== "pro") {
    return NextResponse.json(
      { error: "AI-generated promo banners are a Pro-plan feature.", code: "PLAN_REQUIRED" },
      { status: 403 }
    );
  }

  const usage = await checkAndIncrementFeatureUsage(identity.uid, "banner-image", BANNER_IMAGE_DAILY_LIMIT);
  if (!usage.allowed) {
    return NextResponse.json(
      {
        error: `You've generated all ${usage.limit} promo banners on your ${PLANS.pro.name} plan today. It resets at midnight UTC.`,
        code: "RATE_LIMITED",
      },
      { status: 429 }
    );
