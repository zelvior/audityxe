...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Session-expiration guard: onIdTokenChanged also fires on Firebase's
  // periodic silent token refresh. If that refresh ever fails while a
  // user was previously signed in (revoked/disabled account, or a real
  // token expiration Firebase couldn't silently renew), force a clean
  // sign-out instead of leaving stale UI with a dead token.
  useEffect(() => {
  ...
  // After a user clicks the verification link in a new tab, Firebase's
  // client SDK does NOT auto-update user.emailVerified on the original
  // tab — it's cached from the last token refresh. Re-check it whenever
  // this tab regains focus so "verify your email" doesn't get stuck true
  // forever even after the account is actually verified.
  useEffect(() => {
  ...
  // Deliberately swallows "user not found" / "invalid email" so the
  // caller always sees the same generic outcome regardless of whether
  // the address is registered — otherwise this endpoint becomes a way
  // to enumerate which emails have an account (a real, common
  // vulnerability in naive password-reset implementations).
  const sendPasswordReset = useCallback(async (email: string) => {
    try {
      ...
      if (code === "auth/user-not-found" || code === "auth/invalid-email") {
        return; // silently succeed from the caller's point of view
      }
