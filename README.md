...
### 21-area deep audit breakdown (`result.modules`)

SEO, Performance, Security Headers, **SSL/TLS Certificate** (live TLS
handshake), **Email Authentication** (live DNS SPF/DKIM/DMARC),
**Server Hardening** (dangerous HTTP methods, exposed config files,
directory listing), Accessibility, Mobile Responsiveness, UX/UI,
**AI-Generated / Vibe-Coded Pattern Detection** (judged on combined
signal severity, not any single occurrence), Technical Stack, HTML
Structure, Meta Tags, Sitemap & Robots.txt, Structured Data, Broken
Links, Image Optimization, Third-Party Scripts, Social Metadata, Basic
Monetization Setup, and **Browser-Rendered Audit** (real Chrome, via
PageSpeed Insights — see below).
