...
import { Copy, Download, Share2, Mail, Check, FileText } from "lucide-react";
import { AuditResult } from "@/lib/types";
import { generateAuditPdf } from "@/lib/pdf-export";
import { buildAuditExportPayload } from "@/lib/export-payload";
...
export default function AuditActionBar({ result }: { result: AuditResult }) {
  ...
  function handleExport() {
    const exportable = buildAuditExportPayload(result);
  ...
  function handlePdf() {
    try {
      ...
      setPdfError(true);
      setTimeout(() => setPdfError(false), 1800);
    }
  }

  async function handleShare() {    const text = buildSummaryText(result);
    if (navigator.share) {
      try {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        onClick={handlePdf}
        className={buttonClass}
        aria-label={pdfError ? "PDF export failed" : "Export as PDF"}
        title={pdfError ? "PDF export failed \u2014 try again" : "Export as PDF"}
      >
        <FileText size={15} className={pdfError ? "text-rose" : ""} />
      </button>
