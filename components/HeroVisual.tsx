...
export default function HeroVisual() {
  return (
    <div className="hidden lg:flex w-full h-[360px] items-center justify-center select-none" aria-hidden="true">
      {/* Fixed-size coordinate box, exactly 420×360 — matching the SVG's
          ...
          same 420×360 size, the SVG (scaled to fill whatever width the
          grid column happened to give it) and the divs (never scaled)
          would drift apart the moment the container wasn't exactly
          420px wide. Separately, the check→results connectors used to
          loop back on themselves (visible as the tangled circle around
          the top/bottom rows) because their bezier control points used
          fixed pixel offsets that could land past each other on the
          short top/bottom rows — smoothPath() below fixes that by
          placing both control points at fixed fractions of the actual
          distance instead. */}
      <div className="relative w-[420px] h-[360px] shrink-0">
        <svg width={420} height={360} viewBox="0 0 420 360" className="absolute inset-0 overflow-visible" fill="none">
          <defs>
