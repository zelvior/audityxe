...
import { AuditModule, AuditModuleFinding } from "@/lib/types";
...
function FindingIcon({ finding }: { finding: AuditModuleFinding }) {
  // Unverifiable takes priority over status — a "warn" that's really
  // "we couldn't check this" shouldn't look the same as a confirmed
  // problem, or people end up chasing a fix for something that was
  // never actually confirmed broken.
  if (finding.unverifiable) return <HelpCircle size={14} className="text-text-secondary shrink-0 mt-0.5" />;
  if (finding.status === "pass") return <CheckCircle2 size={14} className="text-emerald shrink-0 mt-0.5" />;
  if (finding.status === "warn") return <AlertTriangle size={14} className="text-amber shrink-0 mt-0.5" />;
...
function ModuleCard({ module }: { module: AuditModule }) {
  ...
  const scoredFindings = module.findings.filter((f) => !f.unverifiable);
  const passCount = scoredFindings.filter((f) => f.status === "pass").length;
            ...
            <div key={i} className="flex items-start gap-2 text-xs sm:text-sm">
              ...
              <div className="min-w-0">
                ...
                {f.status === "fail" && f.severity === "critical" && (
                  <span className="inline-block mr-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-semibold bg-rose/15 text-rose align-middle">
                    CRITICAL
                  </span>
                )}
                {f.unverifiable && (
                  <span className="inline-block mr-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-semibold bg-surface2 text-text-secondary align-middle">
                    UNVERIFIED
                  </span>
                )}
                {!f.unverifiable && f.confidence && f.confidence !== "high" && (
                  <span className="inline-block mr-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-semibold bg-surface2 text-text-secondary/80 align-middle">
